...
def train(scene_index):
    ...
    for epoch in range(starting_epoch, cfg.MAX_EPOCH):
        ...
        if potential_positives is None:
            ...
            _, _, trusted_positives = VFC.Compute_positive(True, db_vec, [], [], None, weight, sort_weight, indice, epoch)

        else:
            potential_positives, potential_distributions, trusted_positives = VFC.Compute_positive(False, db_vec, potential_positives, potential_distributions, trusted_positives, weight, sort_weight, indice, epoch)
