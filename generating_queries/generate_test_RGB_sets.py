    ...
    if test:
        if not evaluate_all:
            ...
            for key in range(len(queries_sets[0].keys())):
                ...
                #print("index[0].tolist():"+str(len(index[0].tolist())))
    else:
        pass
...
def generate(scene_index, evaluate_all = False, inside=True):
    if not evaluate_all:
        ...
        base_path = "/mnt/NAS/data/cc_data/2D_RGB_real_full3"
    #runs_folder = cfg.scene_names[scene_index]
    #runs_folder = "Goffs"
    pre_dir = base_path #os.path.join(base_path, runs_folder)
    ...
    if not evaluate_all:
        #print("df_files_train:"+str((df_files_train)))
